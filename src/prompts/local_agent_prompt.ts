...
const AGENT_TOOL_CALLING_BEST_PRACTICES_BLOCK = `<tool_calling_best_practices>
...
const AGENT_DEVELOPMENT_WORKFLOW_BLOCK = `<development_workflow>
1. **Understand:** Think about the user's request and the relevant codebase context. Use \`grep\` and \`list_files\` extensively (in parallel if independent) to understand file structures, existing code patterns, and conventions. Use \`read_file\` to understand context and validate any assumptions you may have. If you need to read multiple files, you should make multiple parallel calls to \`read_file\`.
...
${AGENT_TOOL_CALLING_BEST_PRACTICES_BLOCK}

${AGENT_FILE_EDITING_TOOL_SELECTION_BLOCK}

${AGENT_DEVELOPMENT_WORKFLOW_BLOCK}
