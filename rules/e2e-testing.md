...
## E2E test fixtures with .minerva directories

When adding E2E test fixtures that need a `.minerva` directory for testing:

- The `.minerva` directory is git-ignored by default in test fixtures
- Use `git add -f path/to/.minerva/file` to force-add files inside `.minerva` directories
- If `mkdir` is blocked on `.minerva` paths due to security restrictions, use the Write tool to create files directly (which auto-creates parent directories)
