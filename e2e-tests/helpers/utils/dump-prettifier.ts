    ...
    .map((message) => {
      const content = Array.isArray(message.content)
        ? JSON.stringify(message.content)
            // Normalize attachment paths (dynamic MD5 hashes in .minerva/media)
            .replace(
              /path: [^"]*?[/\\]{1,2}\.minerva[/\\]{1,2}media[/\\]{1,2}[a-f0-9]+\.\w+/g,
              "path: [[ATTACHMENT_PATH]]",
            )
            // Also normalize .minerva/media paths in escaped attribute format
            .replace(
              /[/\\]{1,2}\.minerva[/\\]{1,2}media[/\\]{1,2}[a-f0-9]{6,}\.\w+/g,
              "/.minerva/media/[[ATTACHMENT_HASH]]",
            )
        : message.content
