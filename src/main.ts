...
export async function onReady() {
  ...
  protocol.handle("dyad-media", async (request) => {
    ...
    // Format: dyad-media://media/{app-path}/.minerva/media/{filename}
    ...
    const mediaDir = path.resolve(path.join(appPath, INTERNAL_MEDIA_DIR_NAME));
    const resolvedPath = path.resolve(path.join(mediaDir, filename));

    // Security: ensure the resolved path stays within the app's .minerva/media directory
